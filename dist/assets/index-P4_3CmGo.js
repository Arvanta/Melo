...
`;const ct=new URLSearchParams(location.search).get("panel");ct&&(document.documentElement.classList.add("panel-window",`panel-${ct}`),document.body.classList.add("panel-window",`panel-${ct}`));var Ae,Ce;if(ut&&ct){Q(async()=>{const{getCurrentWindow:a}=await import("./window-BWd3Cypy.js");return{getCurrentWindow:a}},__vite__mapDeps([8,7,1,0])).then(({getCurrentWindow:a})=>{const n=a();ca(n,"melo-geo-panel-"+ct),n.onCloseRequested(()=>{J("melo:panel-closed",ct)}),window.addEventListener("beforeunload",()=>{J("melo:panel-closed",ct)})});const t=document.getElementById("win-"+ct),e=((Ae=t==null?void 0:t.querySelector(".float-title"))==null?void 0:Ae.innerHTML)||"",i=((Ce=t==null?void 0:t.querySelector(".float-body"))==null?void 0:Ce.innerHTML)||"";Ne.innerHTML=`
