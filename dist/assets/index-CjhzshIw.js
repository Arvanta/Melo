...
`);s&&(It=document.createElement("style"),It.id="melo-custom-skin",It.textContent=s,document.head.appendChild(It));const l=Be(t);let h="";const p=t.match(/<body[^>]*>([\s\S]*?)<\/body>/i);p?h=p[1]:h=t.split(/<\/style>/i).pop()||"";const c=document.createElement("div");c.innerHTML=h;const g=c.querySelector("#lumi-player");if(g&&(h=g.innerHTML),l&&h.trim().length>20){const x=h.trim();a.innerHTML=x,e&&e("Skin applied"),setTimeout(()=>{var M,B;(M=window.__LUMI_REBIND__)==null||M.call(window);const f=window.__LUMI_AUDIO__;f&&window.__LUMI_REBIND_VISUALIZER__&&window.__LUMI_REBIND_VISUALIZER__(f),(B=window.__LUMI_REBIND_MAIN__)==null||B.call(window)},40)}else s&&e&&e("Skin CSS applied");localStorage.setItem("lumi-custom-skin",t),localStorage.setItem("lumi-custom-skin-isFull",l?"1":"0")}function jt(t,e=!0){It&&(It.remove(),It=null);const a=document.getElementById("playerCard");a&&a._originalHTML&&(a.innerHTML=a._originalHTML,setTimeout(()=>{var n,s;(n=window.__LUMI_REBIND__)==null||n.call(window);const i=window.__LUMI_AUDIO__;i&&window.__LUMI_REBIND_VISUALIZER__&&window.__LUMI_REBIND_VISUALIZER__(i),(s=window.__LUMI_REBIND_MAIN__)==null||s.call(window)},40)),localStorage.removeItem("lumi-custom-skin"),localStorage.removeItem("lumi-custom-skin-isFull"),localStorage.setItem("melo-active-skin-id","default"),e&&Y("melo:skin-changed","default"),t&&t("Switched to Default Melo skin")}async function Ce(){if(Tt)try{const{invoke:t}=await N(async()=>{const{invoke:a}=await import("./core-DhEqZVGG.js");return{invoke:a}},[]),e=await t("list_installed_skins");if(Array.isArray(e)&&e.length>0)return e}catch{}return Ye}async function Pe(t,e){if(Tt)try{const{invoke:i}=await N(async()=>{const{invoke:s}=await import("./core-DhEqZVGG.js");return{invoke:s}},[]),n=await i("read_skin_file",{filenameOrPath:t});if(n&&n.trim().length>0)return qt(n,e),!0}catch{}try{const i=t.startsWith("skins/")?t:`skins/${t}`,n=await fetch(i);if(n.ok){const s=await n.text();return qt(s,e),!0}}catch{}const a=t.replace(/^.*[\\/]/,"");return ve[a]?(qt(ve[a],e),!0):(e&&e(`Could not load skin: ${t}`),!1)}async function _t(t,e,a,i=!0){if(t==="default"){jt(a,i);return}let n=t;t==="compact-pill"||t.startsWith("compact-pill")?n=e==="dark"?"compact-pill-dark.html":"compact-pill-light.html":!n.endsWith(".html")&&!n.endsWith(".htm")&&(n=n+".html"),await Pe(n,a)&&(localStorage.setItem("melo-active-skin-id",t),i&&Y("melo:skin-changed",t))}async function ze(t){if(Tt)try{const{invoke:e}=await N(async()=>{const{invoke:a}=await import("./core-DhEqZVGG.js");return{invoke:a}},[]);await e("open_skins_folder"),t&&t("Opening skins folder...")}catch{t&&t("Could not open skins folder")}else t&&t("Skins are located in the skins/ folder")}function Je(t){const e=document.getElementById("skinUpload"),a=document.getElementById("linkDownloadExample");a&&a.addEventListener("click",s=>{s.preventDefault(),Pe("compact-pill-light.html")});const i=localStorage.getItem("melo-active-skin-id")||"default",n=localStorage.getItem("lumi-theme")||"dark";i!=="default"&&setTimeout(()=>{_t(i,n,void 0,!1)},150),at("melo:theme",s=>{const l=localStorage.getItem("melo-active-skin-id");l&&l!=="default"&&_t(l,s,void 0,!1)}),at("melo:skin-changed",s=>{if(s&&typeof s=="string"){const l=localStorage.getItem("lumi-theme")||"dark";_t(s,l,void 0,!1)}}),e&&e.addEventListener("change",async()=>{var p;const s=(p=e.files)==null?void 0:p[0];if(!s)return;const l=await s.text(),h=s.name;if(Tt)try{const{invoke:c}=await N(async()=>{const{invoke:g}=await import("./core-DhEqZVGG.js");return{invoke:g}},[]);await c("save_custom_skin_file",{filename:h,content:l}),t(`Saved ${h} to skins folder`)}catch{}qt(l,t),localStorage.setItem("melo-active-skin-id",h),Y("melo:skin-changed",h),e.value=""}),document.addEventListener("dragover",s=>{var l;[...((l=s.dataTransfer)==null?void 0:l.types)||[]].includes("Files")&&s.preventDefault()}),document.addEventListener("drop",async s=>{var h;const l=[...((h=s.dataTransfer)==null?void 0:h.files)||[]].find(p=>p.name.endsWith(".html")||p.name.endsWith(".htm"));if(l){s.preventDefault();const p=await l.text();if(p.includes("<style")||p.includes("<html")||Be(p)){const c=l.name;if(Tt)try{const{invoke:g}=await N(async()=>{const{invoke:x}=await import("./core-DhEqZVGG.js");return{invoke:x}},[]);await g("save_custom_skin_file",{filename:c,content:p})}catch{}qt(p,t),localStorage.setItem("melo-active-skin-id",c),Y("melo:skin-changed",c)}}}),window.LumiSkin={applyCustomSkin:qt,resetSkin:jt,applySkinChoice:_t,listInstalledSkins:Ce,openSkinsFolderOnDisk:ze}}const Re=document.querySelector("#app");Re.innerHTML=`
...
<div class="panel-root">
  ...
  <div id="toast" class="toast"></div>
    ...
    <a class="about-link" id="aboutLink" href="https://github.com/Arvanta/Melo" rel="noopener">github.com/Arvanta/Melo ↗</a>`,At.style.display=At.style.display==="none"?"block":"none",(e=document.getElementById("aboutLink"))==null||e.addEventListener("click",a=>{a.preventDefault();const i="https://github.com/Arvanta/Melo";Bt?N(()=>import("./core-DhEqZVGG.js"),[]).then(n=>n.invoke("open_url",{url:i})).catch(()=>window.open(i,"_blank")):window.open(i,"_blank")})});document.addEventListener("click",t=>{!t.target.closest("#aboutPop")&&!t.target.closest("#btnAbout")&&(At.style.display="none")});Bt&&ct?ct==="library"||ct==="playlist"?ce(St,tt):ct==="equalizer"?de(St,tt,{remote:!0}):ct==="lyrics"?ue(St):ct==="settings"&&fe(tt):(je(St,tt),ce(St,tt),de(St,tt),Ge(St),ue(St),Je(tt),fe(tt));tt("Melo 0.3 Beta is ready");
