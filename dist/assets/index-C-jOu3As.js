    ...
    <div class="float-win hidden" id="win-settings" style="left:50%; top:50%; width:600px; height:540px; transform:translate(-50%,-50%); z-index:10;">
      ...
      <div class="float-body" style="padding:0; overflow:auto;">
        <div class="settings-tabs" id="settingsTabs">
          <button class="settings-tab active" data-stab="general"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z"/><circle cx="12" cy="12" r="3"/></svg>${ft("settings.tabs.general")}</button>
          <button class="settings-tab" data-stab="playback"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M10 8l6 4-6 4z"/></svg>${ft("settings.tabs.playback")}</button>
          <button class="settings-tab" data-stab="appearance"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="13.5" cy="6.5" r=".5"/><circle cx="17.5" cy="10.5" r=".5"/><circle cx="8.5" cy="7.5" r=".5"/><circle cx="6.5" cy="12.5" r=".5"/><path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10c.926 0 1.648-.746 1.648-1.688 0-.437-.18-.835-.437-1.125-.29-.289-.438-.652-.438-1.125a1.64 1.64 0 0 1 1.668-1.668h1.996c3.051 0 5.555-2.503 5.555-5.554C21.965 6.012 17.461 2 12 2z"/></svg>${ft("settings.tabs.appearance")}</button>
          <button class="settings-tab" data-stab="shortcuts"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="6" width="20" height="12" rx="2"/><path d="M6 10h.01M10 10h.01M14 10h.01M18 10h.01M6 14h.01M18 14h.01M9 14h6"/></svg>${ft("settings.tabs.shortcuts")}</button>
          <button class="settings-tab" data-stab="about"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg>${ft("settings.tabs.about")}</button>
        </div>

        <!-- GENERAL TAB -->
        <div class="settings-section active" data-panel="general">
          <div class="settings-row">
            <div><div class="label">${ft("settings.general.language.label")}</div><div class="desc">${ft("settings.general.language.desc")}</div></div>
            <select class="settings-select" id="setLanguage">${De.map(t=>`<option value="${t.code}">${t.nativeName}</option>`).join("")}</select>
          </div>
          <div class="settings-row">
            <div><div class="label">${ft("settings.general.tray.label")}</div><div class="desc">${ft("settings.general.tray.desc")}</div></div>
            <div class="switch on" id="swTray" data-key="tray"></div>
          </div>
          <div class="settings-row">
            <div><div class="label">${ft("settings.general.resume.label")}</div><div class="desc">${ft("settings.general.resume.desc")}</div></div>
        ...
        <div class="settings-section" data-panel="playback">
          <div class="settings-row">
            <div><div class="label">${ft("settings.playback.replaygain.label")}</div><div class="desc">${ft("settings.playback.replaygain.desc")}</div></div>
            <div class="switch on" id="swReplayGain" data-key="replayGainGlobal"></div>
          </div>
          <div class="settings-row">
            <div><div class="label">${ft("settings.playback.fadepause.label")}</div><div class="desc">${ft("settings.playback.fadepause.desc")}</div></div>
        ...
        <div class="settings-section" data-panel="appearance">
          ...
          <div class="settings-row">
            <div><div class="label">${ft("settings.appearance.showstop.label")}</div><div class="desc">${ft("settings.appearance.showstop.desc")}</div></div>
...
`;const ht=new URLSearchParams(location.search).get("panel");var Ae,Ce;if(ut&&ht){Y(async()=>{const{getCurrentWindow:a}=await import("./window-BWd3Cypy.js");return{getCurrentWindow:a}},__vite__mapDeps([8,7,1,0])).then(({getCurrentWindow:a})=>{const n=a();ca(n,"melo-geo-panel-"+ht),n.onCloseRequested(()=>{W("melo:panel-closed",ht)}),window.addEventListener("beforeunload",()=>{W("melo:panel-closed",ht)})});const t=document.getElementById("win-"+ht),e=((Ae=t==null?void 0:t.querySelector(".float-title"))==null?void 0:Ae.innerHTML)||"",i=((Ce=t==null?void 0:t.querySelector(".float-body"))==null?void 0:Ce.innerHTML)||"";Ne.innerHTML=`
<div class="panel-root">
  ...
  <div id="toast" class="toast"></div>
    ...
    <a class="about-link" id="aboutLink" href="https://github.com/Arvanta/Melo" rel="noopener">github.com/Arvanta/Melo ↗</a>`,Nt.style.display=Nt.style.display==="none"?"block":"none",(i=document.getElementById("aboutLink"))==null||i.addEventListener("click",a=>{a.preventDefault();const n="https://github.com/Arvanta/Melo";ut?Y(()=>import("./core-DhEqZVGG.js"),[]).then(r=>r.invoke("open_url",{url:n})).catch(()=>window.open(n,"_blank")):window.open(n,"_blank")}))});document.addEventListener("click",t=>{!t.target.closest("#aboutPop")&&!t.target.closest("#btnAbout")&&(Nt.style.display="none")});ut&&ht?ht==="library"||ht==="playlist"?we(_t,dt):ht==="equalizer"?xe(_t,dt,{remote:!0}):ht==="lyrics"?Ee(_t):ht==="settings"&&(Me(),_e(dt)):(ta(_t,dt),we(_t,dt),xe(_t,dt),ea(_t),Ee(_t),ia(dt),_e(dt),Me(),setTimeout(()=>{if(localStorage.getItem("melo-pref-resume")!=="0")try{const t=JSON.parse(localStorage.getItem("melo-resume-state")||"null"),e=window.LumiLibrary,i=window.LumiPlayer;if(!(t!=null&&t.trackId)||!e||!i)return;const a=e.tracks,n=a.findIndex(r=>r.id===t.trackId);if(n===-1)return;i.queue=a,i.loadTrack(n,!1,t.position||0)}catch{}},400));ht||dt("Melo 0.3 Beta is ready");
//# sourceMappingURL=index-C-jOu3As.js.map
