...
3. Get the installers:
   - **Releases** → draft release `v1.0.0` (contains `Melo_1.0.0_x64-setup.exe` and the `.msi`) — publish to keep it permanent. Previous tags (`v0.8.1`, `v0.8.0`, `v0.7.2`, `v0.7.1`, `v0.7.0`, `v0.6.1`, `v0.6.0`, …) are left untouched.
   - **Actions → run → Artifacts → melo-windows** (zip with the same installers).
4. Install `Melo_1.0.0_x64-setup.exe` (installs to standard `C:\Program Files\Melo`).
